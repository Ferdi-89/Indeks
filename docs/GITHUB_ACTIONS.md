...
└─────────┘             └─────────┘             └─────────┘
     ...
     └───────────────────────┼───────────────────────┘
                             │
            ┌────────────────▼────────────────┐
            │  Apakah Semua Lolos/Sukses?     │
            └──────────┬─────────────┬────────┘
                    Ya │             │ Tidak
                       │             │
        ┌──────────────▼───┐     ┌───▼──────────────┐
        │  Kode Siap Merge │     │   Merge Diblokir │
        │  & Auto-Deploy   │     │  Developer Fix   │
        └──────────────────┘     └──────────────────┘
```

---

## 📝 Konfigurasi Alur Kerja (Workflow Configuration)

Berikut adalah rancangan file workflow GitHub Actions yang ditempatkan di `.github/workflows/laravel.yml` pada proyek R-NET.

Workflow ini mencakup:
1.  **PostgreSQL Service Container**: Menjalankan database PostgreSQL sementara di dalam server runner untuk pengujian query SQL nyata.
2.  **Linting**: Memvalidasi kepatuhan kode PHP terhadap standar menggunakan Laravel Pint.
3.  **Vite Build**: Memastikan kompilasi frontend assets (CSS, JS) lolos tanpa eror.
4.  **PHPUnit Testing**: Menjalankan *automated tests* (seperti pengujian controller, model, dan modul pendaftaran).

### Berkas: `.github/workflows/laravel.yml`
