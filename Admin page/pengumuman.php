...
<head>
  ...
  <title>R-NET Admin - Pengumuman</title>
...
<body class="bg-gray-50 text-slate-800 h-screen flex overflow-hidden">
  ...
  <main class="flex-1 flex flex-col overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-8 bg-gray-50">
      <div class="mb-8">
        <h3 class="text-2xl font-bold text-slate-800">Pengumuman</h3>
        <p class="text-sm text-slate-500 mt-1">Kelola pengumuman untuk pelanggan</p>
      </div>

      <!-- Create/Edit Form -->
      <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
        <h4 class="text-lg font-bold text-slate-800 mb-4" id="formTitle">Buat Pengumuman Baru</h4>
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">Isi Pengumuman</label>
            <textarea id="announcementText" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 outline-none transition" rows="4" placeholder="Masukkan isi pengumuman"></textarea>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-2">Tanggal Mulai</label>
              <input type="date" id="startDate" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 outline-none transition">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-2">Tanggal Akhir</label>
              <input type="date" id="endDate" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 outline-none transition">
            </div>
          </div>
          <div class="flex gap-3 pt-2">
            <button onclick="saveAnnouncement()" id="saveBtn" class="px-6 py-3 rounded-lg bg-blue-700 hover:bg-blue-800 text-white font-medium shadow-md transition">Publikasikan</button>
  ...
  <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center modal-overlay hidden">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 modal-content hidden p-6 text-center">
      ...
      <h3 class="text-lg font-bold text-slate-800 mb-2">Hapus Pengumuman Ini?</h3>
      <p class="text-slate-500 text-sm mb-6">Yakin ingin menghapus pengumuman ini?</p>
  ...
  <script>
    ...
    let announcements = [
      {
        id: 1,
        text: "Akan dilakukan maintenance jaringan pada tanggal 30 April 2026 pukul 01.00 - 05.00 WIB.",
        startDate: "2026-04-25",
        endDate: "2026-04-30"
      },
      {
        id: 2,
        text: "Dapatkan diskon 30% untuk pelanggan baru yang mendaftar selama bulan Mei 2026.",
    ...
    function renderAnnouncements() {
      ...
      announcements.forEach(ann => {
        const itemHtml = `
          <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-start justify-between gap-4 transition hover:shadow-md">
            <div class="flex items-start gap-4">
              <div class="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0 text-blue-600 mt-1">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>
              </div>
              <div>
                <p class="text-slate-700 font-medium mb-2">${ann.text}</p>
            ...
            <div class="flex gap-2 flex-shrink-0">
              <button onclick="openEditModal(${ann.id})" class="p-2 rounded-lg hover:bg-green-50 text-green-600 transition" title="Edit">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </button>
              <button onclick="openDeleteConfirm(${ann.id})" class="p-2 rounded-lg hover:bg-red-50 text-red-500 transition" title="Hapus">
    ...
    function saveAnnouncement() {
      ...
      if (editId) {
        ...
        if (idx !== -1) {
          announcements[idx] = { ...announcements[idx], text, startDate, endDate };
        }
        cancelEdit(); // Reset to create mode
      } else {
        // Create new
    ...
    function cancelEdit() {
      editId = null;
      document.getElementById('formTitle').innerText = 'Buat Pengumuman Baru';
      document.getElementById('announcementText').value = '';
      document.getElementById('startDate').value = '';
      document.getElementById('endDate').value = '';

      document.getElementById('saveBtn').innerText = 'Publikasikan';
